...
public class ArticleTopicController extends VerifyToken{
	...
	public Response remove(@HeaderParam("Authorization") String token, String article) {
		if (!this.isLogined)
			return Response.status(200).entity(this.notFoundUser().toString()).build();
		ArticleTopicDao articleTopicDao = (ArticleTopicDaoImp) this.appContext.getBean("articleTopicDao");
		JSONObject result = articleTopicDao.remove(article, Integer.parseInt(this.userId));
		return Response.status(200).entity(result.toString()).build();
	}
	...
	public Response restore(@HeaderParam("Authorization") String token, String article) {
		if (!this.isLogined)
			return Response.status(200).entity(this.notFoundUser().toString()).build();
		ArticleTopicDao articleTopicDao = (ArticleTopicDaoImp) this.appContext.getBean("articleTopicDao");
		JSONObject result = articleTopicDao.restore(article, Integer.parseInt(this.userId));
		return Response.status(200).entity(result.toString()).build();
	}
	...
	public Response delete(@HeaderParam("Authorization") String token, String article) {
		if (!this.isLogined)
			return Response.status(200).entity(this.notFoundUser().toString()).build();
		ArticleTopicDao articleTopicDao = (ArticleTopicDaoImp) this.appContext.getBean("articleTopicDao");
		JSONObject result = articleTopicDao.delete(article, Integer.parseInt(this.userId));
		return Response.status(200).entity(result.toString()).build();
	}
}
